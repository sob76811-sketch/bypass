...
-- SCRIPT PRINCIPAL NATIVO (COMPATIBLE CON LUARMOR / XENO / DELTA)
local function runUnifiedScript()
    ...
    -- ANALIZADOR DE MOVIMIENTO TARGET
    ...
    local function analyzeMotion()
        ...
        local n = #s
        if n < 2 then
            return Vector3.zero, Vector3.zero, 0, 0.25
        end
        ...
        local speed = blendVel.Magnitude
        local confidence = math.clamp(n / Track.maxN, 0.3, 1)
        return blendVel, accel, speed, confidence
    end
    ...
    local function gSC(plr)
        if Config.SkeletonStyle == "Oro" then return Config.Theme.Gold end
    ...
    local minBtnCorner = Instance.new("UICorner") minBtnCorner.CornerRadius = UDim.new(0, 6); minBtnCorner.Parent = minBtn
    ...
    local openBallCorner = Instance.new("UICorner") openBallCorner.CornerRadius = UDim.new(1, 0); openBallCorner.Parent = openBall
    ...
    local function cCCS(titleText, buttonsData, callback)
        ...
        local optCorner = Instance.new("UICorner") optCorner.CornerRadius = UDim.new(0, 6); optCorner.Parent = cycleBtn
    ...
    local configLayout = Instance.new("UIListLayout") configLayout.SortOrder = Enum.SortOrder.LayoutOrder; configLayout.Padding = UDim.new(0, 14); configLayout.Parent = ContentConfig
    ...
    local rpgCorner = Instance.new("UICorner") rpgCorner.CornerRadius = UDim.new(0, 6); rpgCorner.Parent = rpgTogBtn
    ...
    local betaCorner = Instance.new("UICorner") betaCorner.CornerRadius = UDim.new(0, 6); betaCorner.Parent = betaBadge
    ...
    local fovTrackCorner = Instance.new("UICorner") fovTrackCorner.CornerRadius = UDim.new(1, 0); fovTrackCorner.Parent = fovTrack
    ...
    local fovFillCorner = Instance.new("UICorner") fovFillCorner.CornerRadius = UDim.new(1, 0); fovFillCorner.Parent = fovFill
    ...
    local fovKnobCorner = Instance.new("UICorner") fovKnobCorner.CornerRadius = UDim.new(1, 0); fovKnobCorner.Parent = fovKnob
    ...
    local function aFFA(alpha)
        ...
        if FOVCircle and HasDrawing then pcall(function() FOVCircle.Radius = Config.AimFOV end) end
        if FOVFrame then local d = math.floor(Config.AimFOV * 2); FOVFrame.Size = UDim2.new(0, d, 0, d) end
    end
    ...
    local function fFI(input)
        ...
        aFFA((input.Position.X - absPos) / absSize)
    end
    ...
    local slc = Instance.new("UICorner") slc.CornerRadius = UDim.new(0, 6); slc.Parent = sliderCont
    ...
    local sbc2 = Instance.new("UICorner") sbc2.CornerRadius = UDim.new(0, 3); sbc2.Parent = sliderBar
    ...
    local sfc = Instance.new("UICorner") sfc.CornerRadius = UDim.new(0, 3); sfc.Parent = sliderFill
    ...
    local sbtnc = Instance.new("UICorner") sbtnc.CornerRadius = UDim.new(1, 0); sbtnc.Parent = sliderBtn
    ...
    -- OPCIÓN DE AURA LEGENDARIAS EN CONFIG
    ...
    local function rPL()
        ...
        for order, player in ipairs(playerList) do
            ...
            if not playerWrapper then
                ...
                local wrapperCorner = Instance.new("UICorner") wrapperCorner.CornerRadius = UDim.new(0, 6); wrapperCorner.Parent = playerWrapper
                ...
                local teamCorner = Instance.new("UICorner") teamCorner.CornerRadius = UDim.new(0, 4); teamCorner.Parent = teamBtn; teamBtn.Parent = playerWrapper
                ...
                local pinCorner = Instance.new("UICorner") pinCorner.CornerRadius = UDim.new(0, 4); pinCorner.Parent = pinBtn; pinBtn.Parent = playerWrapper
    ...
    local function gOME(player)
        ...
        local labelCorner = Instance.new("UICorner") labelCorner.CornerRadius = UDim.new(0, 5); labelCorner.Parent = espLabel; espLabel.Parent = mainLayoutFrame
                ...
                if char then
                    ...
                    if head and root and hum then
                        ...
                        if hum.Health > 0 and distance <= maxDist then
                                ...
                                if State.invViewEnabled then
                                    ...
                                    if LastInventoryState[player] ~= inventoryString then
                                        ...
                                        for i, toolData in ipairs(tools) do
                                            ...
                                            local lineCorner = Instance.new("UICorner") lineCorner.CornerRadius = UDim.new(0, 4); lineCorner.Parent = line
                            ...
                            -- AURA (VERDE ALIADO / ROJA TARGET / AMARILLA LEGENDARIA SEGÚN CONFIG)
                            if isTeam or isLocked or showLegAura then
                                ...
                                if isTeam then
                                    hl.FillColor = Color3.fromRGB(0, 255, 140) -- Verde exclusivo Aliados
                                elseif isLocked then
                                    hl.FillColor = Color3.fromRGB(255, 0, 0) -- Rojo si lo apuntas
                                else
                                    hl.FillColor = Color3.fromRGB(255, 215, 0) -- Amarillo si tiene Legendaria (y opción ON)
                                end
    ...
    local function gIPN(targetMode, char)
        ...
        -- Pecho siempre al torso
        if targetMode == "Pecho" then
            return torso and torso.Name or (head and "Head" or nil)
        end

        -- Cabeza: SIEMPRE a la cabeza real (puro, sin random)
        if targetMode == "Cabeza" then
            return head and "Head" or (torso and torso.Name or nil)
        end

        -- Mixto (modo seguro contra anti-cheat):
        if targetMode == "Mixto" then
            if hum and hum.Health <= 35 then
                return head and "Head" or (torso and torso.Name or nil)
            end
            if math.random() < 0.82 and torso then
                return torso.Name
            end
            return head and "Head" or (torso and torso.Name or nil)
        end
    ...
    sC(RunService.RenderStepped, function(dt)
        ...
        -- DIBUJO DUAL: INTENTA DRAWING Y RESPALDA CON SCREEN_GUI EN DELTA/MÓVIL
        ...
        if FOVCircle and HasDrawing then
            ...
            if ok then drawingSuccess = true end
        end
        ...
        if FOVFrame then
            ...
            -- Si Drawing no esta soportado o falla en el ejecutor movil, activamos el ScreenGui
            if not drawingSuccess or not HasDrawing then
    ...
    RunService:BindToRenderStep("BlockSpinAimbot", Enum.RenderPriority.Camera.Value + 1, function(dt)
        ...
        if target and target.Character then
            ...
            if part and part:IsA("BasePart") then
                ...
                if Config.RPGAimEnabled then
                    ...
                    elseif rpgMode == "Bacio" then
                        if distanceXZ < 70 then
                            ...
                            if movingAway then
                                futurePos = futurePos + (predVel.Unit * (3 + math.clamp(distanceXZ / 30, 0, 8)))
                            end
                ...
                local alpha = 1
                if Config.RPGAimEnabled then
                    if Config.RPGAimMode == "DURO" then
                        alpha = 1
                    elseif Config.RPGAimMode == "Normal" then
                        alpha = 1 - math.exp(-45 * dt)
                    else
                        alpha = 1 - math.exp(-18 * dt)
                    end
                else
                    if Config.AimSmooth == "Bajo" or Config.AimSmooth == "Duro" then
                        alpha = 1
                    elseif Config.AimSmooth == "Medio" then
                        alpha = 1 - math.exp(-45 * dt)
                    else
                        alpha = 1 - math.exp(-18 * dt)
                    end
                            ...
                            if myChar and myChar:FindFirstChildOfClass("Tool") then
                                ...
                                if not endPos then
                                    ...
                                    if rayResult then
                                        ...
                                        endPos = mouseRay.Origin + (mouseRay.Direction * 2000)
                                    end
                                end
...
local function s()
    ...
    if not ok then 
        warn("[HCP] Error al ejecutar script: " .. tostring(err))
    end
end
